...
export class LogoOptimizer {
    async convertToPNG(buffer, targetSize = 256) {
        try {
    ...
    optimizeSVGAttributes(svgContent) {
        // 确保SVG有正确的viewBox
        if (!svgContent.includes('viewBox') && svgContent.includes('width') && svgContent.includes('height')) {
            ...
            if (widthMatch && heightMatch) {
                ...
                svgContent = svgContent.replace(/<svg([^>]*)>/, `<svg$1 viewBox="0 0 ${width} ${height}">`);
            }
        }
        return svgContent;
    }
    async enhanceImage(buffer, format) {
        try {
            if (format === 'svg') {
    ...
    async createMultipleFormats(buffer, originalFormat, targetSize = 256) {
        const results = [];
        try {
            // PNG格式
...
//# sourceMappingURL=logo-optimizer.js.map